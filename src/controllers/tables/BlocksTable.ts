...
const BlocksTable = {
  ...
  updateOne: async (updateBlock: string, blockName: string, blockSize: number, farm: string ): Promise<QueryResult> => {
    return pool.query('UPDATE blocks SET block_name=$2, block_size=$3, farm=$3 WHERE block_name=$1 RETURNING *', [updateBlock, blockName, blockSize, farm])
  },
  updateGeom: async (updateBlock: string, fencing: any ): Promise<QueryResult> => {
    return pool.query('UPDATE blocks SET fencing=$4 WHERE block_name=$1 RETURNING *', [updateBlock, fencing])
  },
