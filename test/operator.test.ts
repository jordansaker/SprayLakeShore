...
describe('Operator tests', () => {
  ...
  test('GET /operators/:last_name/:first_name should get all operators', async () => {
    const res = await request(baseURL).get('/operators/Saker/Jordan')
    expect(res.status).toBe(200)
  ...
  test('GET /operators/:non-existant_last_name/:first_name should return operator not found', async () => {
    const res = await request(baseURL).get('/operators/Sa/Jordan')
  ...
  test('PUT /operators/:last_name/:first_name should update operator profile', async () => {
  ...
  test('PUT /operators/:non-existant_last_name/:first_name should return operator not found', async () => {
    const res = await request(baseURL).put('/operators/Sa/Jordan').send({ chemCert: false })
    expect(res.status).toBe(404)
    expect(res.body).toBeInstanceOf(Object)
    expect(res.body.message).toBe('Operator not found')
  })

  test('DELETE /operators/:last_name/:first_name should delete operator profile', async () => {
    const res = await request(baseURL).delete('/operators')
    expect(res.status).toBe(200)
    expect(res.body).toBeInstanceOf(Object)
    expect(res.body.message).toBe('Operator deleted')
  })

  test('DELETE /operators/:non-existant_last_name/:first_name should return operator not found', async () => {
    const res = await request(baseURL).delete('/operators/Sa/Jordan')
    expect(res.status).toBe(404)
    expect(res.body).toBeInstanceOf(Object)
    expect(res.body.message).toBe('Operator not found')
  })
